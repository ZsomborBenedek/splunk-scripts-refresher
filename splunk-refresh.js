javascript: (() => {
    ...
    const post_form = html => {
        let parser = new DOMParser()
        let page = parser.parseFromString(html, "text/html")
        const data = new URLSearchParams(new FormData(page.forms[0]));
        fetch(url, {
            method: 'POST',
            body: data,
        }).then(res => {
            return res.status >= 400 ? error_scripts(`Error while posting the form!`) : success_scripts('Scripts reloaded!')
        })
    }
